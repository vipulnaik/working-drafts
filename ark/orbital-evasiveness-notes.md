...
>
> The efficiency then follows from r − 1 = 2^a·u (u odd, L the largest prime power dividing u): e = max(1/u, L/(2^{a−1}u)), so e = 1 exactly when a = 1 and u is a prime power. ℓ = 2 forces a ≥ 2 hence e ≤ 1/2; ℓ = 3 forces 3 | u hence e ≤ 1/3; both give e ≤ 1/6.
>
> Validated two ways, since a class maximum alone would only show the cap is attainable. **Upper:** every one of the 1,112 two- and three-part winners satisfies δ ≤ cap(its own efficiency), with zero exceptions. **Lower:** normalising each class by its cap, all twelve reach 0.98–1.00 with medians 0.84–0.94 and minima 0.45–0.81 — indistinguishable between obstructed and unobstructed classes, so no residual class-dependent effect remains unexplained.
...
2. **(Beat the ladder ceilings.)** δ₀ is no longer an unknown constant: Prop. 5.3 gives the closed form and §5.3's table fixes its value in every residue class mod 12, from 1/4 down to 0.05051, each met by the constructions to within 2%. Two parts remain.

   **(a) Exceed 1/9 strictly at odd n.** The 2c + r family approaches 1/9 and cannot attain it — exact balance needs c = r = n/3, and admissibility requires r ≠ p. Since Theorem E.1 of the companion settles the collapse question only *above* 1/9, this gap is exactly what keeps Open Problem 9(b) open at odd n. Per `arithmetic-of-density.md` §2.1 the place to look is fusion, which beats splitting by a factor of F.

   **(b) Lift the obstructed classes.** The efficiency losses at ℓ = 2 and ℓ = 3 obstruct *these* families, not μ itself. A family with different local structure might avoid them; the one worked instance in range is n = 551 = 256 + 167\* + 128, which uses two *distinct* powers of 2 and sidesteps the equal-block form.

   Both Bateman–Horn systems already supply ~n/log³n representations wherever they are soluble, so no strengthening of sieve input helps with either part. This is a question about mechanisms.
