...
*Working notes, July 2026. Status: an asymptotic framework with proof sketches, together with exact machine computations at n = 10 and a campaign in progress at n = 12 (code, checkpoints, and logs accompany this note). Everything stated is either proved here, proved in the companion `enumeration-proof.md`, or cited; the two arithmetic residues are collected as Open Problem 9, and the conjectural number-theoretic inputs are confined to Part II. Intended as a starting point for others.*
...
**5.5 The ledger against μ.** An earlier version of this section measured the *family menu* of `mu_fast.py` to n = 10⁴ against a 1/12 diagnostic threshold, and built a structural explanation of the parity gap on the scarcity of splits n = 2^a + r. Both are superseded. The menu's figures overstated the odd shortfall about threefold, and the structural explanation is simply not how odd n are served: of the 548 odd values reaching density 1/12, **461 use no even part at all** and 339 are a single fused class, with the 2^a + r shape accounting for fewer than 10%. The measurements, and the corrected account of which configurations serve odd n, are in Part I of the companion. What survives is the following, measured against μ itself.
