...
> **Conjecture (global density floor).** For every composite non-prime-power n, **μ(n) ≥ C(n,2)/50**, i.e. δ(n) ≥ 0.02 — and asymptotically **δ(n) ≥ (5 − 2√6)/2 − o(1) = 0.050510…**, the class-11 cap. The extremal class is n ≡ 11 (mod 12), the one carrying both local obstructions; the scan's floor is **0.02504 at n = 3239**, unchanged across a scan to **10⁶**. The branch-and-bound of that section has taken the true minimum to **0.029282 at n = 3059** and reduced the open question to three values — n = 3239, 3479, 8927 — all, like every candidate before them, n ≡ 11 (mod 12). Derivation and the falsification test in `arithmetic-of-density.md` §5.
...
2. **(Beat the ladder ceilings.)** δ₀ is no longer an unknown constant: Prop. 5.3 gives the closed form and §5.3's table fixes its value in every residue class mod 12, from 1/4 down to 0.05051, each met by the constructions to within 2%. What remains has been sharpened by refuting one half of it.

   **(a) Raise the odd-n guarantee above 1/9 — REFUTED.** This asked for a constant above 1/9 valid on odd n, which would let Theorem E.1 of the companion settle the collapse there wholesale. No such constant exists: **54.3% of the odd n in the computed table have δ(n) < 1/9**, and those are exact values of μ, not shortfalls of a family. The share grows with n — 37.0% below 800, 59.9% in [800, 1600), 64.8% in [1600, 2299) — so the route is closed permanently. Open Problem 9(b) must be settled by promoting E.3(ii) directly.

   **(b) Lift the obstructed classes.** Still open. The ℓ = 2 and ℓ = 3 efficiency losses obstruct *these* families, not μ; a family with different local structure might avoid them. The one worked instance in range is n = 551 = 256 + 167\* + 128, which uses two *distinct* powers of 2 and sidesteps the equal-block form. Both Bateman–Horn systems already supply ~n/log³n representations wherever soluble, so no strengthening of sieve input helps: this is a question about mechanisms.
