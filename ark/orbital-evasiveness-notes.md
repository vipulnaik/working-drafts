...
*Working notes, July 2026. Status: an asymptotic framework with proof sketches, together with exact machine computations at n = 10 and a campaign in progress at n = 12 (code, checkpoints, and logs accompany this note). Two companions accompany this note: **`enumeration-proof.md`**, which classifies the admissible group shapes and proves the enumeration correct, complete and attained; and **`arithmetic-of-density.md`**, which identifies the arithmetic conditions on n that control the answer and sets up the Hardy–Littlewood and Bateman–Horn machinery behind them. Everything stated is proved here, proved in one of those, or cited; the two arithmetic residues are collected as Open Problem 9, and the conjectural number-theoretic inputs are confined to Part II. Intended as a starting point for others.*
...
**Where the pieces stand.** §10 assesses; §11 lists the open problems, of which one is sharp and finite (an exhaustive single-shape search at n = 6) and the rest are genuinely hard — with the enumeration's two residues (Open Problem 9) now known to sit in the same low-density regime as the odd-n ladder of Open Problem 2.
...
**Consequence for the extremality claim.** With Lemmas A–C in hand, what §3 states as Theorem 3.1 is no longer a classification problem about solvable permutation groups. The configuration space they permit is finite and explicitly bounded, every member of it is realised by an explicit group, and the resulting bound is attained. The structural gap once inherited from the Singer step (§3) is closed: it bore only on configurations that lose anyway, and the companion's Part E′ now proves that. What is left is whether the enumeration can be pruned to a shortest sufficient list — a question of cost, free above density 1/16 and open only in a low-density tail (Open Problem 9).
...
Writing x = c/n and e = orb(r,t)/C(r,2) for the foreign block's efficiency, the orbital sizes are C(c,2) ≈ x²n²/2 for each intra term, c² for the cross between the two c-blocks, cr for the cross to the foreign block, and e·C(r,2) for the foreign intra term. Hence

> **Proposition 5.3 (the odd-n ceiling, closed form).** δ(x) = min( x², 2x(1−2x), e(1−2x)² ), maximised at the balance point. At **e = 1** this gives **1/9** at x = 1/3; at **e = 1/2**, **1/(2+√2)² ≈ 0.0858** at x = 1/(2+√2); at **e = 1/3**, **≈ 0.0718**.
...
The question the menu could not settle was whether odd n are genuinely poorer or merely poorly served. Since μ(n) = B(n) is known exactly for n ≤ 2007 (§2.6), it can be asked of μ directly, and the answer is **both, in measurable proportions**.

*Odd n are genuinely poorer.* Over the 1,672 computed values the median density is **0.2266 for even n against 0.1104 for odd** — a factor of two in the truth, not in the template. So the parity split is a fact about μ, and the three-block mechanism's weakness is not merely a failure of imagination.

*But the menu overstated it about threefold.* Against the 1/12 diagnostic, the menu put 0.5% of even and **65.4%** of odd composites below the line; μ puts 0.9% of even and **19.3%** of odd. Roughly two thirds of the apparent odd deficit was the menu's shortfall rather than μ's. Likewise the "widening with n" that §5.5 reports (36.5% → 53.9% → 65.4%) is much gentler at the level of μ: odd median density runs 0.1434, 0.1101, 0.1037 across thirds of the range while the even median stays flat at 0.2265 — a real decline, but flattening rather than accelerating.
...
*The parity signal is threshold-dependent, which is why 1/12 was the diagnostic.* Measured instead against δ₀^even = 1/4 — attained only when a balanced safe-prime split exists — **80.7% of even and 81.0% of odd** values fall short, and the parity signal vanishes entirely. So statements about "how many n are weak" say as much about the threshold as about n; the useful thresholds are the ones with mechanisms attached, which is what the next paragraph uses.

*Three thresholds line up by parity, and this is why the odd case is hard everywhere at once.* Below 1/9 — the threshold above which the companion's Theorem E.1 settles the collapse outright — sit 4.4% of even values against **52.0% of odd**. Below 1/16, the threshold above which Corollary F.3 gives k ≤ 3 for free, sit 0.1% of even against 4.0% of odd. So both residues of Open Problem 9 are essentially even-free and concentrate in exactly the odd, low-density regime that Open Problem 2 is about; they are one obstruction viewed three ways, and δ₀^odd > 1/9 would clear all three.

One caveat retained: these figures are for n ≤ 2007, and the density questions above are precisely about whether the pattern persists. The proven ceiling beyond the computed range is B₀ (§2.3), not ⌊C(n,2)/2⌋.

> **Proposition 5.2′ (top rung, both parities).** Unconditionally: μ(n) ≥ δ₀·C(n,2) for every n admitting the relevant representation (checkable per n; two-block for even n, covering-chain for odd n), with δ₀^{even} = 1/4 and δ₀^{odd} ≈ 0.0718, rising to ≈ 0.1111 on n ≡ 1, 9 (mod 12), per Prop. 5.3 — all densities relative to C(n,2). Conditionally (Hardy–Littlewood for the three systems): the exceptional set is finite, hence μ(n) = Θ(C(n,2)) for all sufficiently large n — with an *ineffective* threshold, so this asserts eventual evasiveness only. All-n statements at specific values belong to §8's finite methods.

## 6. The converse: μ encodes prime distribution

By Theorem 3.1, Cor. 3.2 is an equivalence, so lower bounds on μ yield additive prime theorems — and `arithmetic-of-density.md` §3 gives the singular series for the two systems this route actually passes through: μ = ω(n^{3/2}) for all large n implies binary-Goldbach-difficulty representation theorems; μ = Θ(C(n,2)) for all sufficiently large n is equivalent to finiteness of the exceptional sets of the Hardy–Littlewood systems of §5 (two-block for even n, covering-chain three-block for odd n) — the parity split and the effectivity caveat of §5.4 both propagate here: the equivalence is between eventual statements, ineffective on both sides. The scope limit stands: μ probes exactly the Hardy–Littlewood/Bateman–Horn stratum, nothing finer; and the equivalence's one potential leak is the extremality claim of Theorem 3.1, with wreath forms included on the group side.
...
1. **(Extremality — now Theorem 3.1, and no longer open.)** The wreath-inclusive extremal form is proved: Theorems 2.2–2.3 give the block structure, §2.4 the coherence conditions, Part G.2 of the companion the absorption of tower depth, and Part E the converse construction. Exotic imprimitive solvable towers are ruled out not by hypothesis but because tower depth contributes nothing beyond the fusion count. This entry is retained only to record the two traps: any layer-assignment argument must admit block-swaps in the **top q-group** and not only in the cyclic middle layer, or it will miss the wreath forms and understate μ; and the Singer step from prime-power degree to affine-line type is **false** (§3). What is left of the old keystone is arithmetic and confined to low density — Open Problem 9.
2. **(Beat the odd-n ceilings.)** δ₀^odd is no longer an unknown constant: Prop. 5.3 gives the closed form δ(x) = min(x², 2x(1−2x), e(1−2x)²) for the family n = 2c + r, and `local_solubility.py` fixes the available efficiency e by residue class — **1/9 on n ≡ 1, 9 (mod 12), ≈ 0.0858 on n ≡ 3, 7, and ≈ 0.0718 on n ≡ 5, 11**, each met by the constructions to within 2%. So the problem is now sharply posed in two parts.

   **(a) Exceed 1/9 strictly.** The 2c + r family approaches 1/9 and cannot attain it — exact balance needs c = r = n/3, but r is the foreign prime and c the p-characteristic block, and admissibility requires r ≠ p. Since Theorem E.1 of the companion settles the collapse question only *above* 1/9, this gap is exactly what keeps Open Problem 9(b) open at odd n. Per `arithmetic-of-density.md` §2.1 the place to look is fusion, which beats splitting by a factor of F; the target is a configuration fusing at odd n more aggressively than n = 3·(prime power) allows.

   **(b) Lift the ℓ = 2 and ℓ = 3 classes.** The efficiency losses at n ≡ 3 (mod 4) and n ≡ 2 (mod 3) are local obstructions to *this* family, not to μ. A different family — unequal p-blocks, a fused pair, or a foreign block whose twist is a higher q-power — might avoid them. The one worked instance in range is n = 551 = 256 + 167\* + 128, which uses two *distinct* powers of 2 and so sidesteps the equal-block form entirely.

   Note that the older three-block chain analysis, which gave 0.0486 via the (4,6) chain, described a family of shape (one p-block, two foreign primes); the enumeration selects that shape exactly once in 1,672 values, and its ceiling is superseded by the above.
...
9. **(Two arithmetic residues in the enumeration — both confined to low density.)** The configuration enumeration of §2.6 is complete, finite and realised, and the bound it computes is **attained**: μ(n) = B(n), proved rather than observed at every computed value and, via the lower-bound form of the certificate, at every composite non-prime-power n ≤ 10,000 and all but two such n ≤ 100,000. The apparatus is Parts E′–E″ and F of `enumeration-proof.md` and is not repeated here. What remains is two questions, and the striking fact is that both have collapsed into the *same* regime — the low-density tail that Open Problem 2 is also about.

   **(a) Minimality: is k ≤ 3?** Proposition F.1 gives k ≤ 3 free wherever δ > 1/16, which covers 98.3% of the computed table; the open content is the tail below that threshold, 28 of 1,672 computed values. Three things are known about any proof there. Counting cannot supply it — the refinement k + (√2 − 1)f ≤ 1/√δ over f fused parts still yields only 4 at the density floor, and the bound is saturated by equal parts. Nor can domination: three-part winners beat the best two-part configuration by a median factor of 1.69 and up to 4.86, so they are not perturbations of two-part ones and a proof must instead *produce* a strong ≤3-part decomposition at the given n. And it cannot assume the two p-parts are equal, since n = 551 = 256 + 167\* + 128 wins with distinct powers of one prime. The supply question that remains has the same shifted-prime character as the density questions below.

   **(b) The fallback exclusion as a theorem, not a certificate.** Every branch of the exclusion is closed by theorem except one. Above density 1/9 the whole question dies (Theorem E.1), which settles it unconditionally on the n = 2·(prime power) family and conditionally for even n; and the s = 3 branch is a single dead pair, the s = 2 repunit branch is absolutely capped, and the s = 2 safe-prime branch is resolved outright whenever the configuration is the bare pair. What is open is that last branch **with a leftover**, where the re-reading that resolves the bare pair is unavailable — at leftover exactly c it would require two blocks of the same prime to be foreign, which admissibility forbids. The two values below 100,000 that the certificate cannot settle, n = 50,817 and n = 89,697, are exactly this shape and are the concrete targets. The binding obstruction is again the odd-n density floor: δ₀^odd ≈ 0.049 and ≈ 0.028 both fall below the 1/9 threshold, so **raising δ₀^odd above 1/9 would close this wholesale** — which is Open Problem 2.

   Where to look, per `arithmetic-of-density.md`: (a) asks whether the four-part cap 1/16 is ever the best available, which requires ω(n) ≥ 3 together with no good two- or three-part representation — a triple coincidence whose predicted frequency is computable heuristically and would be strong evidence even without a proof; (b) lives exactly where the three-part family is optimal, which is why it is an odd-n problem and why δ₀^odd > 1/9 would close it.

   Two orientation points. Pushing δ cannot deliver "almost all n": δ ≥ 1/4 holds at only 19.2% of computed values and δ > 1/9 at 76.3%, and the parity split of §5.5 is what concentrates the residues on odd n, so density-1/4 is a minority property rather than a generic one. And the asymptotic residue is not an independent difficulty: the absolute caps that close the capped branches are O(n^{3/2}), so those branches die unconditionally exactly above μ(n) = ω(n^{3/2}) — the §4 barrier, the same wall as everything else in Part II.
