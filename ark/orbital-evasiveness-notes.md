...
>
> This is a **purely arithmetic, unconditional** upper bound: a max–min over partitions of n into prime powers, with each part's capacity given by a divisor recursion. It is refined by the coherence conditions of §2.4 and evaluated by the enumeration of §2.6.
...
The enumeration is implemented in `mu_enumerate.py` and analysed in the companion document `enumeration-proof.md`, which classifies the admissible structures and bounds the resulting search: at most 1/√δ orbits, blocks of size at least δn, fusion counts at most 1/δ, and termination certified at the first orbit count no larger configuration can beat. Its output is an **unconditional** upper bound, validated to **n = 1736** at a measured cost of about n^2.9 per value. Moreover every configuration the enumeration admits is realised by an explicit group whose orbital sizes are forced rather than chosen, so the bound is **attained**: μ(n) = B(n). One caveat survives — where Lemma C strictly reduces a p-characteristic twist, the unconditional scoring over-estimates what the construction reaches, so attainment there is empirical; that configuration never wins in the computed range. The arguments behind the classification are written out there but have had no independent scrutiny, and one step originally asserted as a sketch turned out to be false, so the numerical agreement should be read partly as evidence that the reasoning is sound rather than only as a check on the code.
...
*Effect on the bound.* Carrying the bottom prime through the capacity recursion — so that only p-power orbits receive C(s,2), foreign primitive orbits receive the q-power-twist value of Lemma B′, and everything else recurses over q-power block counts — is what turns Theorem 2.3's assumption-free ceiling into a bound the constructions actually meet, at every value computed below 1540 except the 173 where the constructions themselves fall short (§2.6).

**Consequence for ★.** With Lemmas A–C the remaining content of ★ is no longer a classification problem about solvable permutation groups. The configuration space they permit is finite and explicitly bounded, every member of it is realised by an explicit group, and the resulting bound is attained. What is left is whether the enumeration can be pruned to a shortest sufficient list — a question of cost — and one structural gap inherited from the Singer step (§3), which bears on tightness at configurations that do not win.
...
**What therefore remains open** is narrower than Claim 3.1 as stated: not a classification of solvable permutation groups, but two bookkeeping questions about the configuration enumeration of §2.6 — whether it is *minimal* as well as complete, and whether every configuration it admits is realised by an actual group. Both leave the bound valid; neither is a structural conjecture. Realisability is settled — every admitted configuration is an explicit group — so the bound is attained and μ(n) = B(n). The Singer-cycle step in the argument above is known to fail in general — an irreducible cyclic-by-q subgroup of GL(a,p) need not lie in ΓL(1,p^a), the extraspecial group 3^{1+2} inside GL(3,7) being a counterexample — but the bound does not depend on it, for the reasons set out in `enumeration-proof.md`.
...
9. **(Two residual questions about the enumeration.)** The configuration enumeration of §2.6 is complete, finite, and realised — every admitted configuration is an explicit group, so the bound is attained and μ(n) = B(n). Two things remain, neither affecting validity. **Minimality:** no argument prunes the permitted configurations to a shortest sufficient list, which is what the n^{O(1/√δ)} cost reflects; empirically no winning configuration below n = 1736 uses more than two p-characteristic classes, more than two foreign primes, or more than one fused class, and proving any of the three would shorten the search substantially. **Partial-capacity stabilisers:** for a primitive affine orbit 𝔽_c ⋊ H with H cyclic-by-q whose cyclic-layer image has order d, is the minimum ±H-orbit on 𝔽_c∖{0} at most 2d? An affirmative answer would make attainment unconditional as well as validity. See `enumeration-proof.md`.
