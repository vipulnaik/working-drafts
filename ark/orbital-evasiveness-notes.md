...
**What therefore remains open** is narrower than Claim 3.1 as stated: not a classification of solvable permutation groups, but two bookkeeping questions about the configuration enumeration of §2.6 — whether it is *minimal* as well as complete, and whether every configuration it admits is realised by an actual group. Both leave the bound valid and merely conservative; neither is a structural conjecture. The Singer-cycle step in the argument above is known to fail in general — an irreducible cyclic-by-q subgroup of GL(a,p) need not lie in ΓL(1,p^a), the extraspecial group 3^{1+2} inside GL(3,7) being a counterexample — but the bound does not depend on it, for the reasons set out in `enumeration-proof.md`.
...
9. **(Minimality and realisability of the enumeration.)** The configuration enumeration of §2.6 is complete and finite, and yields an unconditional upper bound. Two questions remain, both affecting tightness rather than validity: whether the enumeration can be pruned to a shortest sufficient list, which would reduce its n^{O(1/√δ)} cost; and whether every configuration it admits is realised by an actual Oliver group, since it reasons about shapes rather than groups and grants each the most permissive chain. See `enumeration-proof.md`.
