...
> **Conjecture (global density floor).** For every composite non-prime-power n, **μ(n) ≥ C(n,2)/50**, i.e. δ(n) ≥ 0.02 — and asymptotically **δ(n) ≥ (5 − 2√6)/2 − o(1) = 0.050510…**, the class-11 cap. The extremal class is n ≡ 11 (mod 12), the one carrying both local obstructions; the observed floor is **0.02504 at n = 3239**, unchanged across a scan to **10⁶**, with the low-density behaviour confined to small n. The branch-and-bound of that section has reduced the open question to **27 explicitly listed values**, all n ≡ 11 (mod 12), between 2915 and 17363. Derivation and the falsification test in `arithmetic-of-density.md` §5.
...
>
> The efficiency then follows from r − 1 = 2^a·u (u odd, L the largest prime power dividing u): η = max(1/u, L/(2^{a−1}u)), so η = 1 exactly when a = 1 and u is a prime power. ℓ = 2 forces a ≥ 2 hence η ≤ 1/2; ℓ = 3 forces 3 | u hence η ≤ 1/3; both give η ≤ 1/6.
>
...
*Three thresholds line up by parity, and this is why the odd case is hard everywhere at once.* Below 1/9 — the threshold above which the companion's Theorem E.1 settles the collapse outright — sit 4.8% of even values against **54.3% of odd**. Below 1/16, the threshold above which Corollary F.3 gives k ≤ 3 for free, sit 0.1% of even against 5.6% of odd. So both residues of Open Problem 9 are essentially even-free and concentrate in exactly the odd, low-density regime that Open Problem 2 is about; they are one obstruction viewed three ways, and δ₀^odd > 1/9 would clear all three.
