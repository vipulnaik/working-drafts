...
One correctness point worth stating, since it looks like a gap. A twist of q-power order could sit in the top q-group rather than the cyclic layer, and Lemma C would then not bind it — so one might expect the code to have to model twist *placement*. It does not need to: the q′-part of any twist is forced into Γ₁ (because Γ/Γ₁ is a q-group), so Lemma C applies precisely to that part, while the q-part is automatically coprime to every foreign prime r since q | r − 1 forces q ≠ r. Taking the twist order to be the largest divisor of c − 1 coprime to the foreign primes is therefore exactly right, and the apparent refinement is vacuous.
