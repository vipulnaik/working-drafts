...
> Over the current table: **no candidate at any of the 1,848 values, 0 inconclusive**, with **1,390 (75.2%) settled by Theorem E.1 alone** and the other 458 by the search — every one of those 397 uncovered because δ ≤ 1/9, never because a Cap(a) binds. So μ(n) = B(n) is proved at each computed n, independently of tie-breaking, and B_refined = B_safe follows rather than being separately measured.
...
From `mu_table_safe_v2.csv` — n = 6 … 2212, unconditional mode, 1,848 rows:

- every value certified; no internal inconsistency; **no row violates the Proposition F.1 stopping rule** (checked directly: 1/√δ ≤ K on all 1,848);
- **independently re-derived**: recomputing m\* from each row's witness string by the G.3 formulas, in a separate implementation, reproduces `mu_bound` on **1,848 of 1,848** rows, with parts summing to n, foreign parts prime and unfused and pairwise distinct, p-characteristic parts powers of p, and every F a q-power;
- the unconditional fallback is invoked on the optimum at **0 of 1,848** values, so the bound is unconditional throughout;
- **certification level, not orbit count.** The `certified_K` column records the K at which the Part F iteration halts, which is one more than the part count it rules out. Its distribution is {2: 347, 3: 1043, 4: 419, 5: 39}, but the **actual part counts are {1: 733, 2: 875, 3: 240}** — no winner uses more than three parts. An earlier version of this list called the column an orbit count and concluded that "K = 5 is exactly the predicted ceiling, reached by exactly the hardest rows"; that is wrong twice over. At the weakest density 0.0418 (n = 575) Proposition F.1 permits **four** orbits and the winner there uses **one**. Nothing in range has five orbits;
...
**The low-density tail, where minimality is still open.** Corollary F.3 gives k ≤ 3 wherever δ > 1/16, so the open content of Part J item 1 is confined to the tail below that threshold: **39 of the 1,848 computed values** (1.7%), namely n = 323, 455, 527, 551, 575, 725, 731, 851, … . Proposition F.1 permits four parts at every one of them, and the winners there use one, two and three parts in 7, 10 and 11 cases respectively — so nothing in the tail approaches the permitted ceiling either.
...
1. **Minimality: k ≤ 3 in the low-density tail.** Corollary F.3 makes this free wherever δ > 1/16 — 97.9% of the computed table — so what is open is the tail below that threshold, measured in Part I at 28 of 1,848 values. (How much of n > 2007 lies below 1/16 is itself governed by the open density questions, so this is a per-n statement, not a global one.)
...
2. **The collapse for odd n at low density: the global promotion.** Part E′ closes every branch but one — s = 3 outright (E.4), the s = 2 repunit family by absolute cap (E.3(iii)), s = 1 by Cap(a) (E.1), and everything at δ > 1/9 (E.1) — and E″ certifies the collapse at all but two composite non-prime-power n ≤ 100,000 from lower bounds alone. The single surviving branch is **s = 2 with c a safe prime**, and within it E.3(ii) now resolves the bare pair outright, so what is open is only the case **with a leftover**.
   ...
   Two things to keep in view. The per-n frontier now stands at n ≤ 100,000 and extends cheaply, since E″'s lower-bound pass dominates the cost and is cached. And "almost all n" is the wrong target — the density thresholds of Part I show δ ≥ 1/4 at only 18.8% of computed values, so density-1/4 is a minority property; proving thinness of the exceptional set would land on the same shifted-prime condition, a Hardy–Littlewood-type statement of the tier this framework is trying to avoid depending on.
...
4. **Extending the numerical range.** Validation currently reaches **n = 2212** at a measured cost of about n^2.9 per value; see Part H for projections. This extends the certificate's per-n coverage and tests the implementation, but does not test the classification, whose consequences do not vary with n.
