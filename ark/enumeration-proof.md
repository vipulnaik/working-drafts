...
1. **Minimality of the enumeration.** No argument prunes the permitted configurations to a shortest sufficient list. Costs running time only, never validity.

   The target suggested by the data is **k ≤ 3**, against the 4 that Proposition F.1 permits at the observed density floor. Proposition F.2 shows the counting route cannot get there — F.1 is saturated by equal parts, and separating fused from unfused parts still yields only 4 — so a proof must use the prime-power structure and the constraint n = Σ F_i c_i. Part I records where to start: essentially every three-part winner is "one foreign prime plus two equal unfused p-parts", and the reason a fourth part loses is that it shrinks the budget per part; what is missing is that the freed points be reabsorbable at the same n. The three older sub-narrowings — at most two p-characteristic classes, at most two foreign primes, at most one fused class — all follow from k ≤ 3 and remain individually worth proving, since each removes a different axis.
...
2. **The collapse for odd n at low density.** Theorem E.1 settles the sandwich whenever δ(n) > 1/9, hence unconditionally on n = 2·(prime power) and conditionally for even n. It does **not** reach odd n: the ladder constants δ₀^odd ≈ 0.049 (3∤n) and ≈ 0.028 (3|n) both fall below 1/9, so s ≤ 3 survives and the s ∈ {2, 3} branches are untouched. Two routes:
   ...
   - a direct argument at s = 2 and s = 3. For orientation, s = 2 forces c = 2r + 1 with r prime and c a prime power (r Sophie Germain) *and* r − 1 carrying a prime-power divisor ≳ 3δn (r near-safe) — a double shifted-prime condition of heuristic density ~1/log²n, thin but not obviously empty.
