...
That collapse is proved at every computed value (`fallback_cert.py`: 1,672 of 1,672, 0 inconclusive), and settled by theorem rather than by search at 76.3% of them. It is **unconditional** on the infinite family n = 2·(prime power), **conditional** for even n, and **open for odd n** at density below 1/9. Numerical validation currently reaches **n = 2007**, 1,672 composite values. Open items are collected at the end.
...
From `mu_table_safe.csv` — n = 6 … 2007, unconditional mode, 1,672 rows:

- every value certified; no internal inconsistency; **no row violates the Proposition F.1 stopping rule** (checked directly: 1/√δ ≤ K on all 1,672);
- **independently re-derived**: recomputing m\* from each row's witness string by the G.3 formulas, in a separate implementation, reproduces `mu_bound` on **1,672 of 1,672** rows, with parts summing to n, foreign parts prime and unfused and pairwise distinct, p-characteristic parts powers of p, and every F a q-power;
- the unconditional fallback is invoked on the optimum at **0 of 1,672** values, so the bound is unconditional throughout;
- **certification level, not orbit count.** The `certified_K` column records the K at which the Part F iteration halts, which is one more than the part count it rules out. Its distribution is {2: 321, 3: 954, 4: 369, 5: 28}, but the **actual part counts are {1: 678, 2: 793, 3: 201}** — no winner uses more than three parts. An earlier version of this list called the column an orbit count and concluded that "K = 5 is exactly the predicted ceiling, reached by exactly the hardest rows"; that is wrong twice over. At the weakest density 0.0418 (n = 575) Proposition F.1 permits **four** orbits and the winner there uses **one**. Nothing in range has five orbits;
...
- the density floor is stable rather than eroding across the range: minimum 0.041812 at n = 575, maximum 0.499749 at n = 1994 (`2x997`, Theorem 2.1), median 0.1995. Thresholds relevant to Part J item 2: δ ≥ 1/4 at 321 values (19.2%), δ > 1/9 at 1,275 (76.3%);
- the **δ > 1/9 theorem of Part J item 2 settles 1,275 of the 1,672 values (76.3%) outright**, with no search needed. The other 397 rely on the exhaustive certificate, and *all* 397 are uncovered for the same reason — δ ≤ 1/9. The Mersenne branch never binds: no n in range has Cap(a) ≥ B(n) for any applicable exponent. Only Cap(2) = 6, Cap(3) = 21, Cap(5) = 155 and Cap(7) = 1143 arise, with tightest margins B/Cap = 1.43 at n = 15, 1.63 at n = 63 and 1.90 at n = 20. So the entire residual difficulty is low density, not the Mersenne coincidence.

Winning configuration shapes, over all 1,672 rows: **737** use one p-characteristic part with one foreign prime; **678** a single fused class; **200** two p-characteristic parts with one foreign prime; **56** a fused class with a foreign prime; **1** uses two foreign primes. These are the same five shapes reported at n ≤ 1306 (553 / 542 / 127 / 46 / 1) — **no sixth shape appears** in the extended range, which is the more informative fact. The last two are the ones a hand-built family menu is most likely to miss, since they combine features that are natural to treat separately. The unique two-foreign winner is n = 1175 = 641\* + 277 + 257\* at (p, q) = (277, 2), binding on the Fermat prime 257 at 257·128 = 32,896.
...
2. **The ΓL(1) conclusion for partial capacity — the question as posed is ANSWERED NO, and the caveat it guarded is now certified in range.**
   ...
   *The question, and its answer.* The item previously reduced to: for a primitive affine orbit 𝔽_c ⋊ H with H cyclic-by-q and the image of H in the cyclic layer of order d, is the minimum ±H-orbit on 𝔽_c∖{0} at most 2d? **No** — and the counterexample is the group Part B already introduces. For E = 3^{1+2} on 𝔽₇³ the ±E-orbits have sizes {18, 54} while d = |Z(E)| = 3, so the minimum is **18 against 2d = 6**. An affirmative answer would have made orb(c, d) a valid per-part upper bound in general, and hence made attainment follow from B_refined = B_safe as a theorem. It is not, so that route is closed. Part B records the arithmetic, together with the reason the under-statement never propagates: it needs small foreign blocks, hence density O(1/n). No n with μ(n) > B_refined(n) is known, and none can arise by this mechanism for large n.

   *What replaces it.* Attainment is secured not by rescuing the formula but by showing the fallback never bites the optimum. Since the two scores agree wherever the fallback does not bite (Part B), this is the same as proving the sandwich collapses. Let W be a configuration containing a p-characteristic part (F, c) and a foreign prime r of W with r | c−1; put s = (c−1)/r and δ = B(n)/C(n,2). The foreign part's own intra term satisfies orb(r,t) ≤ C(r,2), so r(r−1) ≥ δ·n(n−1); and c ≤ n − r. Hence s ≤ (n−r−1)/r, and

   > **s ≤ (1 − √δ)/√δ** — so s ≤ 3 at δ ≥ 1/16, s ≤ 2 at δ ≥ 1/9, and **s = 1 as soon as δ > 1/9** (the sharp form: s ≤ 1 needs only r > (n−1)/3, which r² > δn(n−1) delivers once δ > (n−1)/9n).

   > **Theorem (collapse above density 1/9).** If δ(n) > 1/9 then any fallback configuration attaining B(n) has s = 1, hence c − 1 = r is prime, and exactly one of:
   >
   > - **p odd.** Then c is odd, c − 1 is even, so r = 2. A foreign block of size 2 contains a single pair, so orb(2, t) = 1 and SAFE(W) ≤ 1. Excluded at every n with B(n) > 1.
   > - **p = 2.** Then c = 2^a and r = 2^a − 1 is a **Mersenne prime**, and d = strip(r, {r}) = 1 — the twist dies outright. Since r − 1 = 2(2^{a−1} − 1) and t is a q-power dividing it, SAFE(W) ≤ orb(r, t) ≤ **Cap(a) := (2^a − 1)·max(2, L(a))**, where L(a) is the largest prime-power divisor of 2^{a−1} − 1. This is an absolute constant, independent of F and of n.
   >
   > So the collapse holds unless B(n) ≤ Cap(a) for some Mersenne exponent a with 2^{a+1} − 1 ≤ n — an O(log n) check.

   > **Lemma (Cap is small).** For a an odd prime write a − 1 = 2m. Then 2^{2m} − 1 = (2^m − 1)(2^m + 1) with the two factors coprime, so every prime power dividing 2^{a−1} − 1 divides one of them, whence **L(a) ≤ 2^{(a−1)/2} + 1** and **Cap(a) = O(n^{3/2})**. The bound is attained at a = 17, where L = 257 = 2⁸ + 1.

   *Coverage: what this settles and what it leaves.* Four cases, in decreasing strength.

   - **n = 2·(prime power): settled unconditionally, no conjecture at all.** Theorem 2.1 gives δ = (m−1)/(2m−1) → 1/2 > 1/9. An infinite family on which the sandwich provably collapses.
   - **Even n, conditionally: settled.** The two-block ladder gives δ₀^even = 1/4 > 1/9, so granting the even-n Hardy–Littlewood statement the collapse holds at every even n admitting the representation, hence at all sufficiently large even n.
   - **Above exponent 3/2, unconditionally for the s = 1 branch.** Cap(a) = O(n^{3/2}) means that branch dies as soon as δ = ω(n^{−1/2}), i.e. **μ(n) = ω(n^{3/2})** — which is exactly the §4 provability barrier. So the residue of this item is not an independent difficulty; it is the same wall as the rest of Part II. Note that Shparlinski's unconditional Ω(n^{5/4−ε}) gives only δ = Ω(n^{−3/4−ε}) and is *not* enough.
   - **Odd n: not settled by this argument, and the gap is specific.** δ₀^odd ≈ 0.049 (3∤n) and ≈ 0.028 (3|n) both fall **below the 1/9 threshold**, so s ≤ 3 survives and the s ∈ {2, 3} branches are untouched. Closing this needs either a better odd-n construction — Open Problem 2, the 0.049 ceiling of Prop. 5.3 — or a direct argument at s = 2 and s = 3. For orientation, s = 2 forces c = 2r + 1 with r prime and c a prime power (so r Sophie Germain) *and* r − 1 carrying a prime-power divisor ≳ 3δn (so r near-safe): a double shifted-prime condition of heuristic density ~1/log²n, which is why it is thin but not obviously empty.

   *Why "almost all n" is not the right target.* Pushing δ cannot get there: measured over the current table, δ ≥ 1/4 holds at only **19.2%** of values and δ > 1/9 at **76.3%**, and §5.5 of the notes already records that 85.8% of even and 86.9% of odd composites fall short of δ₀ = 1/4. Density-1/4 is a minority property, not a generic one. Proving thinness of the exceptional set instead would land on the shifted-prime condition above — i.e. it would itself be a Hardy–Littlewood-type statement of the same tier, with no verification literature, so the trade buys less than it appears to.

   *How this closes attainment, and what it leans on.* Part E already discharges the hard half: for every admitted configuration it builds an explicit group whose orbital sizes are forced, so B_refined(n) ≤ μ(n) unconditionally. Part E's conclusion is conditional on "the enumeration's score for each part being the one the construction realises", which is exactly the condition that the fallback does not bite. The certificate discharges that antecedent, so the two endpoints of the sandwich meet:

   > B_refined(n) = B_safe(n) = B(n), hence **μ(n) = B(n)** exactly, the lower bound from Part E's construction and the upper bound from the classification.

   Two things this subsumes. It upgrades "the fallback is invoked on the winner at 0 of N values", a statement about the reported witness, into a statement about all configurations, immune to tie-breaking. And it makes the mode agreement a corollary rather than a second measurement, which is what Part B's "0 of 806 rows differ" was recording. Note also the free diagnostic this exposes: **B_safe − B_refined is exactly the width of the interval containing μ(n)**, computable by running both modes, and zero wherever the certificate passes.

   *Caveats on the necessary conditions.* Getting these right took two corrections, both in the direction of the check being too permissive. Without the leftover-size condition, 58 of the then-1,582 values admit candidates; without the leftover-*decomposition* condition, 5 do. The five survivors are all one shape — c a safe prime with r = (c−1)/2, e.g. 359/179 at n = 725, 731 and 719/359 at n = 1457, 1595, 1643 — and are excluded because the leftover is either not of the form (q-power)·(prime power) (187 = 11·17, 517 = 11·47, 565 = 5·113) or is a prime whose q-part twist is trivial (193, 379). Anyone re-deriving this should reproduce those five before trusting the empty result. The check is also not exhaustive if a leftover could itself be two or more parts; that case does not arise anywhere in the current range and is reported when it does.

   *What is still open, and what failure would cost.* Only the asymptotic statement. Nothing here bounds the exceptional set as n → ∞; the certificate is per-n and must be rerun as the table extends. Conditionally on the ladder (δ bounded below) the s-bound forces s = 1 and the Cap(a) argument then excludes all but finitely many n outright, so the asymptotic question is really a question about how small δ can be. And if the certificate ever fails at some n, nothing becomes *wrong*: both endpoints stay valid, the sandwich merely opens, and μ(n) is pinned to an interval instead of a point at that n.
