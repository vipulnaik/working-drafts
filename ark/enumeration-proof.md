...
> **Corollary (the fallback question below 1/9, reduced).** Since s ≤ 1/√δ − 1, **δ > 1/25 forces s ≤ 3** — and every computed value has δ ≥ 0.0418 > 1/25. So at every computed n the fallback question reduces by theorem to: s = 1 (Mersenne, Cap(a)); s = 2 with a ≥ 2 (repunit, Cap′(a)); s = 2 with a = 1 (safe prime, pairwise dominated by E.3(ii)); s = 3 (dead by E.4). The **only branch without an absolute cap is s = 2, a = 1**, and its pairwise domination is what remains to be promoted to a global statement. That promotion — extra foreign primes re-score under (r, r) and may drop — is now the entire theorem-side residue of Part J item 2 within the computed density regime.
...
> Over the current table: **no candidate at any of the 1,672 values, 0 inconclusive**, with **1,275 (76.3%) settled by Theorem E.1 alone** and the other 397 by the search — every one of those 397 uncovered because δ ≤ 1/9, never because a Cap(a) binds. So μ(n) = B(n) is proved at each computed n, independently of tie-breaking, and B_refined = B_safe follows rather than being separately measured.
...
From `mu_table_safe_v2.csv` — n = 6 … 2007, unconditional mode, 1,672 rows:
...
- the density floor is stable rather than eroding across the range: minimum 0.041812 at n = 575, maximum 0.499749 at n = 1994 (`2x997`, Theorem 2.1), median 0.1995. Thresholds relevant to Theorem E.1: δ ≥ 1/4 at 321 values (19.2%), δ > 1/9 at 1,275 (76.3%);
- **Theorem E.1 settles 1,275 of the 1,672 values (76.3%) outright**, with no search needed. The other 397 rely on the exhaustive certificate, and *all* 397 are uncovered for the same reason — δ ≤ 1/9. The Mersenne branch of Theorem E.1 never binds: no n in range has Cap(a) ≥ B(n) for any applicable exponent. Only Cap(2) = 6, Cap(3) = 21, Cap(5) = 155 and Cap(7) = 1143 arise, with tightest margins B/Cap = 1.43 at n = 15, 1.63 at n = 63 and 1.90 at n = 20. So the entire residual difficulty is low density, not the Mersenne coincidence.
...
1. **Minimality: k ≤ 3 — free above density 1/16; open only in the low-density tail (28 of the 1,672 computed values).** Proposition F.1 already yields k ≤ 3 wherever **δ > 1/16** (then 1/√δ < 4). This is a per-n statement, not a global one: within the computed table it covers 98.3% of values, and the open content there is exactly the **δ ≤ 1/16 tail — 28 of 1,672 rows**; how much of n > 2007 falls below 1/16 is itself governed by the open density questions (F.1 permits four parts at every one of them; the winners use 1, 2, 3 parts in 7, 10, 11 cases). Two measurements sharpen what a proof there must look like.

   *Margins refute the perturbation route.* Sampling 18 of the three-part winners: B₃/B₂ — the winner against the best ≤2-part configuration at the same n — runs from 1.04 to **4.86** (n = 777, where the Fermat-prime configuration 263\* + 257 + 257 nearly quintuples the best two-part value), median ≈ 1.3. Three-part winners are not marginal improvements on two-part configurations, so symmetrically there is no reason a four-part configuration should be dominated by a small perturbation of itself; a k ≤ 3 proof in the tail must *produce* a strong ≤3-part decomposition arithmetically, and that supply question has the same shifted-prime character as the density questions. Together with Proposition F.2 (counting saturates at 4), the honest status is: **k ≤ 3 is free above density 1/16, and in the tail it is plausibly as hard as the low-density arithmetic itself** — the same regime as item 2.

   *The proof cannot assume equal p-parts.* n = 551 = 256 + 167\* + 128 wins with two distinct 2-powers, so any domination argument must cover same-prime unequal pairs, not just the 199-of-200 equal shape.
...
2. **The collapse for odd n at low density.** Theorem E.1 settles the sandwich whenever δ(n) > 1/9, hence unconditionally on n = 2·(prime power) and conditionally for even n. It does **not** reach odd n: the ladder constants δ₀^odd ≈ 0.049 (3∤n) and ≈ 0.028 (3|n) both fall below 1/9, so s ≤ 3 survives and the s ∈ {2, 3} branches are untouched. Two routes:

   - a better odd-n construction, raising δ₀^odd above 1/9 — this is Open Problem 2 of the notes, the 0.049 ceiling of Prop. 5.3, and is the same obstruction as everything else about odd n;
   - a direct argument at the surviving branches. This is now largely done: **s = 3 is closed outright** (Theorem E.4 — the branch is the single pair (16, 5), SAFE ≤ 10), the **s = 2, a ≥ 2 repunit branch is SAFE-capped** (E.3(iii), Cap′(a) = O(r^{3/2}), per-n O(log n) check), and **s = 2, a = 1 is pairwise dominated** (E.3(ii)). Since δ > 1/25 forces s ≤ 3 and every computed value clears 1/25, the sole theorem-side residue in the computed density regime is **promoting E.3(ii)'s pairwise domination to a global one** in the presence of extra parts — extra foreign primes re-score under the (r, r) reading and may drop, so the promotion needs either a leftover-absorption argument (the certificate's condition (7)–(8) machinery, run structurally rather than per n) or a different dominating configuration.

   Beyond that, only the asymptotic statement is open — and its per-n frontier has moved: the lower-bound certificate of E″ proves the collapse at **every composite non-prime-power n ≤ 10,000** without the table, and extends mechanically (pass 1 is the only cost, ~O(N²/log N); n ≤ 10⁵ is an offline hour, not a research problem). The genuinely open content is the promotion as a *theorem*: cases (α)–(γ) of E″ show it cannot be structural, the Cunningham-chain analysis shows where it is hardest, and the q-pinning mechanism is the likely ingredient of any unconditional argument. Note that "almost all n" is the wrong target — δ ≥ 1/4 holds at only 19.2% of computed values and δ > 1/9 at 76.3%, consistent with §5.5's finding that ~86% of composites of either parity fall short of δ₀ = 1/4, so density-1/4 is a minority property rather than a generic one. Proving thinness of the exceptional set instead would land on the shifted-prime condition above, i.e. on a Hardy–Littlewood-type statement of the same tier with no verification literature.
...
4. **Extending the numerical range.** Validation currently reaches **n = 2007** at a measured cost of about n^2.9 per value; see Part H for projections. This extends the certificate's per-n coverage and tests the implementation, but does not test the classification, whose consequences do not vary with n.
