...
>
> What E.3 does **not** close: promoting the pairwise domination of (ii) to a global one when the configuration carries further parts (extra foreign primes re-score under (r, r) and may drop), and converting (i)'s linear cap on *real groups* into an exclusion of *SAFE candidates*. The s = 3 branch is untouched. These are the live residue of Part J item 2.
...
1. **Minimality: k ≤ 3, now confined to a 28-value tail.** Proposition F.1 already yields k ≤ 3 wherever **δ > 1/16** (then 1/√δ < 4). Over the current table that is 98.3% of values; the open content is exactly the **δ ≤ 1/16 tail: 28 of 1,672 values** (F.1 permits four parts at every one of them; the winners use 1, 2, 3 parts in 7, 10, 11 cases). Two measurements sharpen what a proof there must look like.
...
2. **The collapse for odd n at low density.** Theorem E.1 settles the sandwich whenever δ(n) > 1/9, hence unconditionally on n = 2·(prime power) and conditionally for even n. It does **not** reach odd n: the ladder constants δ₀^odd ≈ 0.049 (3∤n) and ≈ 0.028 (3|n) both fall below 1/9, so s ≤ 3 survives and the s ∈ {2, 3} branches are untouched. Two routes:
   ...
   - a direct argument at s = 2 and s = 3. The s = 2 branch is now structurally characterised by **Theorem E.3**: for a ≥ 2 it is the base-3 repunit-prime family with a linear cap on real groups, and for a = 1 (c a safe prime) the pair is pairwise dominated by its own non-fallback (r, r) reading. What remains at s = 2 is the promotion to global domination with extra parts present, and converting the a ≥ 2 cap on real groups into an exclusion of SAFE candidates; **s = 3 is untouched** — there c − 1 = 3r forces c even, so c = 2^a with 2^a − 1 = 3r, i.e. r = (2^a − 1)/3 with a even (a = 2: r = 1; a = 4: r = 5; a = 6: r = 21 ✗; a = 8: r = 85 ✗; the viable exponents are those with (2^a−1)/3 prime), another thin forced family worth working out first.
