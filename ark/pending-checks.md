...
python3 ladder_verify.py 300000                                             # ladder hypothesis, per n, unconditionally
```

**`ladder_verify.py` is the cheapest high-value run here.** The conditional lower bounds of §5 of the notes hold at any n admitting the representation their family needs, and deciding that at a given n costs O(n/log n) — against n^2.9 for B(n). So the hypothesis can be verified far past the table: currently to 200,000, with the largest failure at n = 28,993 (odd family) and n = 96,568 (even family), so every eligible n above those and below 200,000 is unconditional. Extending to 10⁷ is an afternoon and would make the ladder unconditional over four orders of magnitude more than B(n) is known. This matters because Bateman–Horn is an ineffective asymptotic and can never supply a statement about any particular n.
