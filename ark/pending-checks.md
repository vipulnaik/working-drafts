...
Cost is O(N²/log N), not linear — 23 s to 10⁵, 87 s to 2·10⁵ — so **10⁶ is about half an hour and is the run worth doing**; 10⁷ is multi-day and not worth it without a further reason. (An earlier note here quoted 193 s for 10⁷; that timing was the old binary check, which scanned a narrow fixed window.)
